...
async function apiGetStoreSignups(): Promise<StoreSignup[]> {
  const res = await fetch("/api/admin/store-signups?type=store", {
    method: "GET",
    cache: "no-store",
  });
...
async function apiUpdateSignupStatus(id: string, status: DbSignupStatus): Promise<StoreSignup> {
  const res = await fetch("/api/admin/store-signups/status", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const renderRow = (app: StoreSignup) => {
    ...
    return (
      <tr key={app.id} className="row">
        ...
        <td className="cell note-cell">{note && <div className="sub">{note}</div>}</td>
      ...
      <main className="app-main">
        <div className="page-root">
          <p className="page-lead">
            /signup/creator から送信された店舗向けの signup_applications を表示しています。ステータスを
            approved にすると「店舗の本登録」「users.role の更新」まで自動で行われます。rejected にした場合は申請のみ更新されます。
          </p>
