...
export default function ComposeClient() {
  // hydration/ビルド安全のため、localStorage依存値はマウント後に確定
  const [logicalUserId, setLogicalUserId] = useState<string>("");
  const [currentRole, setCurrentRole] = useState<
    "user" | "therapist" | "store" | "guest"
  >("guest");
  ...
  useEffect(() => {
    try {
      const uid = getCurrentUserId(); // "guest-xxxxx" or UUID
      const role = getCurrentUserRole(); // "user" | "therapist" | "store" | "guest"
      setLogicalUserId(uid);
      setCurrentRole(role as any);
      // therapist のみ後続チェックに入る
      setCheckingStatus(role === "therapist");
    } catch (e) {
      // 失敗時は guest 扱いで継続
      setLogicalUserId("");
  ...
  useEffect(() => {
    ...
    // therapist でも UUID でないなら照会しない（安全側）
