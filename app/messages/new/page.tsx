...
export default function NewMessagePage() {
  ...
  useEffect(() => {
    ...
    // to がOKでも、auth未確定や未ログインの処理は別Effectでやる
    setPageError(null);
  }, [targetUserId]);

  // =========================
  // 2) ログイン判定は Auth user で確定（roleで判定しない）
  // =========================
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      try {
        // to が空/不正の時はここで飛ばさない（まずエラー表示させる）
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const handle =
          u?.name && u.name.trim().length > 0 ? `@${u.name.trim()}` : `@${targetUserId}`;
        ...
        if (resolvedRole === "therapist") {
          ...
          if (!cancelled) {
            if (thErr) console.warn("[MessagesNew] partner therapist fetch error:", thErr);
            if (th) {
